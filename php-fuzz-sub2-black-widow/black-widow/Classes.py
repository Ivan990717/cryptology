...
class Graph:
    ...
    class Edge:
        ...
        def printedge(self):
            return str(self.n1.value.url) +"|"+self.value.printedge()
...
class Form:
    ...
    def printfile(self):
        ...
        return str(self.action) + "|" +str(self.method) + "|" + str(inputs) 
...
class Event:
    ...
    def __repr__(self):
        ...
        return s
    def printfile(self):
        s  = str(self.event) +"|" +str(self.id) +"|" +str(self.tag) +"|" +str(self.addr) +"|" +str(self.event_class)
        return s
...
class Crawler:
                ...
                for edge in self.graph.edges:
                    if edge.visited == False:
                        if edge.value.method == "get":
                            f_get = open("get_req.txt", "a+")
                            f_get.write(edge.printedge()  + "\n")
                            n_gets += 1
                        elif edge.value.method == "form":
                            f_form = open("form_req.txt", "a+")
                            f_form.write(edge.printedge() + "\n")
                            n_forms += 1
                        elif edge.value.method == "event":
                            f_event = open("event_req.txt", "a+")
                            f_event.write(edge.printedge() + "\n")
                            n_events += 1
                print()
                print("----------------------")
    ...
    def fix_form(self, form, payload_template, safe_attack):
        ...
        for parameter in form.inputs:
            ...
            if safe_attack:
                ...
                # List all injectable input types text, textarea, etc.
                if parameter.itype in ["text", "textarea", "password", "email", "hidden", "number"]:
                    # Arm the payload
                    form.inputs[parameter].value = payload
                    self.use_payload(lookup_id, (form,parameter,payload))
    ...
    def use_payload(self, lookup_id, vector_with_payload):
        d = open("data.txt", "a")
        print(vector_with_payload)
        print("\n\n\n\n\n\n\n\n\n")
        d.write(str(vector_with_payload)+"\n")
...
class CrawlEdge:
    ...
    def printedge(self):
        if self.method == "get":
            return str(self.method_data)+"|"+str(self.cookies)
        else:
            return self.method_data.printfile() +"|"+str(self.cookies)
