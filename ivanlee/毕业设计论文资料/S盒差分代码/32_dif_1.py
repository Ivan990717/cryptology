...
def difference_encode(message,key):
    ...
    f=str_xor(guzhang,message[24:28])
    left_plain=message[:24]+f+message[28:32]
    right_plain=message[32:]
...
if __name__ == '__main__':
    plain = "1010110011110011101111011100100110101101011001000111101011100111"
    key = "10111011001010101001011011100001010011011001000100100000011011000110110000010110"
    ...
    for i in range(0,16):
        ...
        if xor1_dif[i*4:i*4+4]!=xor2_dif[i*4:i*4+4]:
            print(xor1_dif[i*4:i*4+4],xor2_dif[i*4:i*4+4],"*")
        else:
            print(xor1_dif[i * 4:i * 4 + 4], xor2_dif[i * 4:i * 4 + 4])
