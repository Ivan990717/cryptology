...
def LFSRinit():  # 用64bit密钥初始3个移位寄存器,分别是19,22,23位
    ...
    key_bin_str = ""
    for i in key:
        tmp = bin(ord(i))[2:].zfill(8)  # 转为2进制，一次返回8位
        print(tmp)
        key_bin_str += tmp
    X = key_bin_str[0:19]
    Y = key_bin_str[19:41]
...
def create_key():
    ...
    for i in range(114):   # A5 / 1用于为每个突发产生114比特的密钥流序列
        ...
        c_y = int(Y[10])
        c_z = int(Z[10])
        print("x8=", c_x, " y10=", c_y, " z10=", c_z)
        print("x8+y10+z10=", c_x ^ c_y ^ c_z)
        print("new_x=", x, " new_y=", y, " new_z=", z)
...
def a5_encode(mess):
    ...
    print("base64加密: ", base64.b64encode(str_cipher.encode('utf-8')))
