...
def getKeyLen(cipherText):  # 获取密钥长度
    ...
    for step in range(3, 10):  # 循环密钥长度
        count = 0
        # range(step,len(cipherText)-step):
...
def getKey(text, length):  # 获取密钥
    ...
    for i in range(length):
        ...
        for j in range(26):
            Sum = 0.00000
            for k in range(26):
                # Ic的值Sum[k] += alphaRate[k]*li[k+j] 的话  就不用后面的切片操作
        ...
        for j in range(len(powLi)):
            print("第", i+1, "组的", chr(97+j), "的频率为", powLi[j])
...
if __name__ == '__main__':
    cipherText = open('D:\密码学课程设计\Vtext.txt', 'r', encoding='utf-8')
    text = cipherText.read()
    cipherText.close()
    text = text.replace('\n', '')
    # cipherText = input("please input the cipher: ")  # 这里要根据文档目录的不同而改变
    plainText = virginiaCrack(text.lower())
    print('the plainText:\n', plainText)
