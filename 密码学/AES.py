class AES:
	...
	def SubBytes(self, State):
		...
		for position in range(16):
			i = State[position] >> 4
			j = State[position] & 0xF
			State[position] = self.S_BOX[i][j]
		return State
	...
	def SubBytes_Inv(self, State):
		...
		for position in range(16):
			i = State[position] >> 4
			j = State[position] & 0xF
			State[position] = self.I_SBOX[i][j]
		return State
	...
	def round_key_generator(self, _16bytes_key):
		...
		for i in range(4, 44):
			temp = w[i-1]
			if not i % 4:
				temp = self.SubWord(self.RotWord(temp)) ^ self.RCon[int(i/4)-1]
			w[i] = w[i-4] ^ temp
		#print(w)
		W = []
		for i in range(11):
			key = (w[4*i] << 96) + (w[4*i+1] << 64) + (w[4*i+2] << 32) + (w[4*i+3])
			W += self.num_2_16bytes(key)
		return W
	def AddRoundKey(self, State, RoundKeys, index):

		# 异或轮密钥
		return self._16bytes_xor(State, RoundKeys[16 * index: 16 * (index+1)])
	...
	def aes_encrypt(self, plaintext_list, RoundKeys):
		...
		print("round[ 0 ].字节转换   ", hex(aes._16bytes2num(State)))
		for i in range(1,10):
			State = self.SubBytes(State)
			print("round[", i, "].字节转换   ", hex(aes._16bytes2num(State)))
			State = self.ShiftRows(State)
			print("round[", i, "].行移位     ", hex(aes._16bytes2num(State)))
			State = self.MixColumns(State)
			print("round[", i, "].列混合     ", hex(aes._16bytes2num(State)))
			State = self.AddRoundKey(State, RoundKeys, i)
			print("round[", i, "].轮密钥加   ", hex(aes._16bytes2num(State)))
		State = self.SubBytes(State)
		print("round[ 10].字节转换   ", hex(aes._16bytes2num(State)))
		State = self.ShiftRows(State)
		print("round[ 10].行移位     ", hex(aes._16bytes2num(State)))
		State =self.AddRoundKey(State, RoundKeys, 10)
		print("round[ 10].轮密钥加   ", hex(aes._16bytes2num(State)))
		return State

	def aes_decrypt(self, ciphertext_list, RoundKeys):
		State = ciphertext_list
		State = self.AddRoundKey(State, RoundKeys, 10)
		print("初始字节逆转换          ", hex(aes._16bytes2num(State)))
		for Round in range(1, 10):
			State = self.ShiftRows_Inv(State)
			print("round[", Round, "].行逆移位     ", hex(aes._16bytes2num(State)))
			State = self.SubBytes_Inv(State)
			print("round[", Round, "].字节逆转换   ", hex(aes._16bytes2num(State)))
			State = self.AddRoundKey(State, RoundKeys, 10-Round)
			print("round[", Round, "].轮密钥逆加   ", hex(aes._16bytes2num(State)))
			State = self.MixColumns_Inv(State)
			print("round[", Round, "].列逆混合     ", hex(aes._16bytes2num(State)))
		State = self.ShiftRows_Inv(State)
		print("round[ 10].行逆移位     ", hex(aes._16bytes2num(State)))
		State = self.SubBytes_Inv(State)
		print("round[ 10].字节逆转换   ", hex(aes._16bytes2num(State)))
		State = self.AddRoundKey(State, RoundKeys, 0)
		print("round[ 10].轮密逆钥加   ", hex(aes._16bytes2num(State)))
		return State

	def _16bytes2num(self, _16bytes):
		# 16字节list转数字
		return (_16bytes[ 0] << 120) + (_16bytes[ 1] << 112) + (_16bytes[ 2] << 104) + (_16bytes[ 3] << 96) + \
			   (_16bytes[ 4] <<  88) + (_16bytes[ 5] <<  80) + (_16bytes[ 6] <<  72) + (_16bytes[ 7] << 64) + \
			   (_16bytes[ 8] <<  56) + (_16bytes[ 9] <<  48) + (_16bytes[10] <<  40) + (_16bytes[11] << 32) + \
			   (_16bytes[12] <<  24) + (_16bytes[13] <<  16) + (_16bytes[14] <<   8) + (_16bytes[15])

	def num_2_16bytes(self, num):
		# 数字转16字节list
		return [num >> 120 & 0xff, num >> 112 & 0xff, num >> 104 & 0xff, num >> 96 & 0xff,
				num >>  88 & 0xff, num >>  80 & 0xff, num >>  72 & 0xff, num >> 64 & 0xff,
